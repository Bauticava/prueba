...
def launch_setup(context, *args, **kwargs):
    ...
    add_suction_tube = LaunchConfiguration('add_suction_tube', default=False)
    ...
    suction_tcp_offset_xyz = LaunchConfiguration('suction_tcp_offset_xyz', default='"0 0 0"')
    suction_tcp_offset_rpy = LaunchConfiguration('suction_tcp_offset_rpy', default='"0 0 0"')
    ...
    dof_value = int(dof.perform(context))

    def lc_to_bool(lc):
        return lc.perform(context) in ('True', 'true', '1')
    ...
    moveit_config = MoveItConfigsBuilder(
        ...
        add_suction_tube=add_suction_tube,
        ...
        suction_tcp_offset_xyz=suction_tcp_offset_xyz,
        suction_tcp_offset_rpy=suction_tcp_offset_rpy,
    ...
    xarm_traj_controller = '{}{}_traj_controller'.format(prefix_value, xarm_type)
    servo_yaml['command_out_topic'] = '/{}/joint_trajectory'.format(xarm_traj_controller)

    collision_links = [f"{prefix_value}link_base"]
    collision_links.extend(f"{prefix_value}link{i}" for i in range(1, dof_value + 1))
    collision_links.append(f"{prefix_value}link_eef")
    if lc_to_bool(add_suction_tube):
        collision_links.append(f"{prefix_value}suction_tube_link")
    servo_yaml['collision_check_robot_link_names'] = collision_links
    servo_params = {"moveit_servo": servo_yaml}
    controllers = []
    if add_gripper.perform(context) in ('True', 'true') and robot_type.perform(context) != 'lite':
        controllers.append('{}{}_gripper_traj_controller'.format(prefix_value, robot_type.perform(context)))
    elif add_bio_gripper.perform(context) in ('True', 'true') and robot_type.perform(context) != 'lite':
        controllers.append('{}bio_gripper_traj_controller'.format(prefix_value))
    ...
    if joystick_type.perform(context) == "3":
        print("CREATING SPACENAV NODE")
        joy_node = Node(
            package='spacenav',
            ...
            output='screen',
            parameters=[{
                "device":"/dev/input/by-id/usb-3Dconnexion_SpaceMouse_Wireless_BT-event-joystick",
                "use_sim_time": True,
                "angular_scale/x": 0.01,
                "angular_scale/y": 0.01,
                "angular_scale/z": 0.01,
                "linear_scale/x": 0.01,
                "linear_scale/y": 0.01,
                "linear_scale/z": 0.01,
                }],
        ...
        print("CREATING casual joystick")
