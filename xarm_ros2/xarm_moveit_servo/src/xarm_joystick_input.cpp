...
JoyToServoPub::JoyToServoPub(const rclcpp::NodeOptions &options)
    : Node("joy_to_twist_publisher", options), dof_(7), ros_queue_size_(10),
      joystick_type_(JOYSTICK_XBOX360_WIRED), initialized_status_(10),
      joy_topic_("/joy"),
      cartesian_command_in_topic_("/servo_server/delta_twist_cmds"),
      joint_command_in_topic_("/servo_server/delta_joint_cmds"),
      robot_link_command_frame_("link_base"), ee_frame_name_("link_eef"),
      planning_frame_("link_eef") {
  // init parameter from node
  _declare_or_get_param<int>(dof_, "dof", dof_);
  _declare_or_get_param<int>(ros_queue_size_, "ros_queue_size",
                             ros_queue_size_);
  ...
  _declare_or_get_param<std::string>(
      ee_frame_name_, "moveit_servo.ee_frame_name", ee_frame_name_);
  _declare_or_get_param<std::string>(
      planning_frame_, "moveit_servo.planning_frame", planning_frame_);

  if (cartesian_command_in_topic_.rfind("~/", 0) == 0) {
    cartesian_command_in_topic_ =
        "/servo_server/" + cartesian_command_in_topic_.substr(
                               2, cartesian_command_in_topic_.length());
  }
  ...
  // 10);
  RCLCPP_WARN_STREAM(this->get_logger(), "PLANNING FRAME: " << planning_frame_);
}
...
void JoyToServoPub::_declare_or_get_param(T &output_value,
  ...
  RCLCPP_INFO_STREAM(this->get_logger(), "Found parameter - " << param_name
                                                              << ": "
                                                              << output_value);
}
...
bool JoyToServoPub::_convert_xbox360_joy_to_cmd(
  ...
  if (buttons[XBOX360_BTN_A] || buttons[XBOX360_BTN_B] ||
      buttons[XBOX360_BTN_X] || buttons[XBOX360_BTN_Y] || axes[cross_key_lr] ||
      axes[cross_key_fb]) {
    ...
    joint->velocities.push_back(
        (buttons[XBOX360_BTN_B] - buttons[XBOX360_BTN_X]) * 1);
    joint->joint_names.push_back("joint" + std::to_string(dof_ - 1));
    joint->velocities.push_back(
        (buttons[XBOX360_BTN_Y] - buttons[XBOX360_BTN_A]) * 1);
    return false;
  }
...
void JoyToServoPub::_switch_command_type(int command_type) {
  if (command_type == command_type_)
    return;
  switch (command_type) {
  case 0: {
    // switch_request_->command_type =
    // moveit_msgs::srv::ServoCommandType::Request::JOINT_JOG; auto result =
    // switch_input_->async_send_request(switch_request_); if
    // (result.get()->success)
    // {
    //     command_type_ = command_type;
    //     RCLCPP_INFO_STREAM(this->get_logger(), "Switched to input type:
    //     JOINT_JOG");
    // }
    // else
    // {
    //     RCLCPP_WARN_STREAM(this->get_logger(), "Could not switch input to:
    //     JOINT_JOG");
    // }
    ...
    if (rclcpp::spin_until_future_complete(node_, result_future) ==
      ...
      if (result->success) {
        ...
        RCLCPP_WARN(node_->get_logger(),
                    "Could not switch input to: JOINT_JOG");
      }
    } else {
      RCLCPP_ERROR(node_->get_logger(),
                   "Failed to call service /servo_server/switch_command_type");
    }
    break;
  }
  case 1: {
    // switch_request_->command_type =
    // moveit_msgs::srv::ServoCommandType::Request::TWIST; auto result =
    // switch_input_->async_send_request(switch_request_); if
    // (result.get()->success)
    // {
    //     command_type_ = command_type;
  ...
  case 2: {
    // switch_request_->command_type =
    // moveit_msgs::srv::ServoCommandType::Request::POSE; auto result =
    // switch_input_->async_send_request(switch_request_); if
    // (result.get()->success)
    // {
    //     command_type_ = command_type;
    //     RCLCPP_INFO_STREAM(node_->get_logger(), "Switched to input type:
    //     POSE");
    // }
    // else
    // {
    //     RCLCPP_WARN_STREAM(node_->get_logger(), "Could not switch input to:
    //     POSE");
    // }

    switch_request_->command_type =
        moveit_msgs::srv::ServoCommandType::Request::POSE;
    auto result_future = switch_input_->async_send_request(switch_request_);
    if (rclcpp::spin_until_future_complete(node_, result_future) ==
        rclcpp::FutureReturnCode::SUCCESS) {
      auto result = result_future.get();
...
bool JoyToServoPub::_convert_spacemouse_wireless_joy_to_cmd(
  ...
  twist->twist.linear.x = scaleFactor * axes[SPM_STICK_X];
  twist->twist.linear.y = scaleFactor * axes[SPM_STICK_Y];
  twist->twist.linear.z = scaleFactor * axes[SPM_STICK_Z];

  twist->twist.angular.x = scaleFactor * axes[SPM_STICK_ROLL];
  twist->twist.angular.y = scaleFactor * axes[SPM_STICK_PITCH];
  twist->twist.angular.z = scaleFactor * axes[SPM_STICK_YAW];
...
void JoyToServoPub::_joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg) {
  ...
  case JOYSTICK_XBOX360_WIRELESS: // xbox360 wireless
    if (msg->axes.size() != 8 || msg->buttons.size() != 11)
      return;
    pub_twist = _convert_xbox360_joy_to_cmd(msg->axes, msg->buttons, twist_msg,
                                            joint_msg);
    break;
  case JOYSTICK_SPACEMOUSE_WIRELESS: // spacemouse wireless
    if (msg->axes.size() != 6 || msg->buttons.size() != 2)
      return;
    pub_twist = _convert_spacemouse_wireless_joy_to_cmd(msg->axes, msg->buttons,
                                                        twist_msg);
    break;
  default:
    return;
  ...
  if (pub_twist) {
    ...
    _filter_twist_msg(twist_msg, 0.0);
